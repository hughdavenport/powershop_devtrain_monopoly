...
RSpec.describe HousePurchasesController, type: :controller do
  ...
  describe "POST #create" do
    ...
    context "when it is our turn" do
      ...
      before do
        purchase_house_service
      end

      # Mock out service that is going to be called
      let(:purchase_house_service) do
        class_double("PurchaseHouse").as_stubbed_const.tap do |purchase_house_service|
          expect(purchase_house_service).to receive(:new).with(game: game, property: property).and_return(service)
        end
      end

      let(:service) do
        double("PurchaseHouse").tap do |service|
      ...
      context "with valid game state" do
        ...
        it "has a success notice" do
          expect(flash[:notice]).to eq "House purchased"
        end
