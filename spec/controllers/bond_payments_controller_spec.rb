...
RSpec.describe BondPaymentsController, type: :controller do
  # Inputs
  let(:game_id) { "1" }
  ...
  describe "POST #create" do
    ...
    context "when it is our turn" do
      ...
      context "and we have to roll the dice" do
        ...
        it "should have a flash message" do
          expect(flash[:danger]).to eq "Already rolled"
        end
      end

      context "when we haven't already rolled the dice" do
        ...
        let(:pay_bond_service) do
          class_double("PayBond").as_stubbed_const.tap do |pay_bond_service|
            expect(pay_bond_service).to receive(:new).with(game: game).and_return(service)
          end
        end

        let(:service) do
          double("PayBond").tap do |service|
        ...
        context "with valid game state" do
          ...
          it "has a success notice" do
            expect(flash[:notice]).to eq "Bond paid"
          end
