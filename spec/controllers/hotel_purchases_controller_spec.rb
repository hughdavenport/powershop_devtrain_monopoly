...
RSpec.describe HotelPurchasesController, type: :controller do
  # Inputs
  let(:game_id) { "1" }
  ...
  describe "POST #create" do
    ...
    context "when it is not out turn" do
      ...
      before { post :create, {game_id: game_id, username: username, property: property}, valid_session }
    ...
    context "when it is our turn" do
      ...
      before do
        purchase_hotel_service
      end

      # Mock out service that is going to be called
      let(:purchase_hotel_service) do
        class_double("PurchaseHotel").as_stubbed_const.tap do |purchase_hotel_service|
          expect(purchase_hotel_service).to receive(:new).with(game: game, property: property).and_return(service)
        end
      end

      let(:service) do
        double("PurchaseHotel").tap do |service|
      ...
      context "with valid game state" do
        ...
        it "has a success notice" do
          expect(flash[:notice]).to eq "Hotel purchased"
        end
