...
RSpec.describe CardDrawsController, type: :controller do
  # Inputs
  let(:game_id) { "1" }
  ...
  describe "POST #create" do
    ...
    context "when it is our turn" do
      ...
      let(:draw_card_service) do
        class_double("DrawCard").as_stubbed_const.tap do |draw_card_service|
          expect(draw_card_service).to receive(:new).with(game: game, card: card).and_return(service)
        end
      end

      let(:service) do
        double("DrawCard").tap do |service|
      ...
      context "with valid game state" do
        ...
        before { post :create, {game_id: game_id, card: card, username: username}, valid_session }

        it "has a success notice" do
          expect(flash[:notice]).to eq "Card drawn"
        end
      ...
      context "passing in our own card" do
        let(:card) { "Advance to Go" }
        let(:return_value) { true }
