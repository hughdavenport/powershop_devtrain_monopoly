...
RSpec.describe ShuffleCommunityChestCards, type: :service do
  let(:game) do
  ...
  before do
    community_chest_cards_shuffled_event
  end

  let(:community_chest_cards_shuffled_event) do
    class_double("CommunityChestCardsShuffled").as_stubbed_const.tap do |community_chest_cards_shuffled|
      expect(community_chest_cards_shuffled).to receive(:new).with(no_args).and_return(event)
    end
  end

  let(:event) do
    double("CommunityChestCardsShuffled").tap do |event|
      expect(event).to receive(:can_apply?).and_return(can_apply)
    end
  end

  subject(:service) { ShuffleCommunityChestCards.new(game: game) }
