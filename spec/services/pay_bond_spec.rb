...
RSpec.describe PayBond, type: :service do
  let(:game) do
  ...
  let(:bond_paid_event) do
    class_double("BondPaid").as_stubbed_const.tap do |bond_paid|
      expect(bond_paid).to receive(:new).with(no_args).and_return(event)
    end
  end

  let(:event) do
    double("BondPaid").tap do |event|
      expect(event).to receive(:can_apply?).and_return(can_apply)
    end
  end

  subject(:service) { PayBond.new(game: game) }
