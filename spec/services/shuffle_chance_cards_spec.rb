...
RSpec.describe ShuffleChanceCards, type: :service do
  let(:game) do
  ...
  before do
    chance_cards_shuffled_event
  end

  let(:chance_cards_shuffled_event) do
    class_double("ChanceCardsShuffled").as_stubbed_const.tap do |chance_cards_shuffled|
      expect(chance_cards_shuffled).to receive(:new).with(no_args).and_return(event)
    end
  end

  let(:event) do
    double("ChanceCardsShuffled").tap do |event|
      expect(event).to receive(:can_apply?).and_return(can_apply)
    end
  end

  subject(:service) { ShuffleChanceCards.new(game: game) }
