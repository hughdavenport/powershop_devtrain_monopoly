...
RSpec.describe PurchaseHouse, type: :service do
  let(:game) do
  ...
  before do
    house_purchased_event
  end

  let(:house_purchased_event) do
    class_double("HousePurchased").as_stubbed_const.tap do |house_purchased|
      expect(house_purchased).to receive(:new).with(property: property).and_return(event)
    end
  end

  let(:event) do
    double("HousePurchased").tap do |event|
      expect(event).to receive(:can_apply?).and_return(can_apply)
    end
  end

  subject(:service) { PurchaseHouse.new(game: game, property: property) }
